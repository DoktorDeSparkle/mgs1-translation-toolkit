...
2. **[scripts] Unknown chunk type crashes DEMO/VOX loading on Integral disc** — `demoClasses.py` was calling `root.append("unknownChunk", {...})` which passes two arguments to `ET.Element.append()` (only takes one), crashing on Integral chunk types that hit the `case _:` fallback. Fixed by wrapping in `ET.Element()` — now `root.append(ET.Element("unknownChunk", {"type": str(chunkType), "length": str(length)}))` at demoClasses.py:640.

3. **[USA disc] Radio call loading broken** — loading the first radio call on the USA version resulted in corrupted face-loading graphics and the call failing to start, due to a format difference between JP and USA RADIO.DAT parsing.

## Known Bugs

1. **[scripts/GUI format mismatch] iseeva JSON discards VOX_CUES offsets** — `RadioDatTools.py` (lines 866-873) emits its iseeva-style radio JSON as 2-level `calls: {callOffset: {subOffset: text}}`, using a recursive `.//SUBTITLE` search that flattens away the enclosing VOX_CUES container. The GUI's in-memory `radioAlteredJson` uses 3-level `{callOffset: {voxOffset: {subOffset: text}}}` (where `voxOffset` falls back to `callOffset` for direct-subtitle calls without VOX_CUES). This means `.mtp` files written by the GUI are not directly consumable by `xmlModifierTools.injectSubtitles`, and iseeva JSONs produced by the CLI cannot be loaded into the GUI without a conversion step. Reconcile the two formats later — likely by dropping the vox level in the GUI to match the CLI, since subtitle offsets are already globally unique within a call. Touches `_extractIseevaCallsFromXml` (mainwindow.py:99), all `radioAlteredJson` / `radioOriginalJson` read/write sites, and the compile path that calls `XMT.injectSubtitles` / `injectUserPrompts` / `injectSaveBlocks` / `injectCallNames`.

2. **[ZMovie playback window] Subtitle size inconsistent across window resize** — `ZmoviePlaybackWindow` in `mainwindow.py` configures libVLC's freetype subtitle renderer with `--freetype-fontsize=<px>` (a fixed pixel size). The video scales with the window but the subtitle text does not, so subs appear proportionally oversized when the window is small and undersized when it's large. Fix options: (a) switch to `--freetype-rel-fontsize=<enum>` which scales with the video (values are coarse: ~20=smallest, 6=largest), or (b) listen for `resizeEvent` and rebuild the libVLC instance with a recomputed fontsize each time (slow — instance rebuild reloads the media). (a) is cleaner if the granularity is acceptable.
